...
<?php if ( have_comments() ) : ?>
	...
	<?php comments_number(__('No Comment','book-store'), __('One Comment','book-store'), __('% Comments','book-store') );?>
	</div>
	...
	$comment_form = array( 
		'fields' => apply_filters( 'comment_form_default_fields', array(
			'author' => '<div class="comment-form-author">' .
						'<label for="author">' . __( 'Name', 'book-store' ) . '</label> ' .
			...
			'email'  => '<div class="comment-form-email">' .
						'<label for="email">' . __( 'Email', 'book-store' ) . '</label> ' .
						( $req ? '<span class="required">*</span>' : '' ) .
			...
			'url'    => '<div class="comment-form-url">' .
						'<label for="url">' . __( 'Website', 'book-store' ) . '</label>' .
						'<input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" tabindex="3" />' .						
						'<div class="clear"></div>' .
						'</div><!-- #form-section-url .form-section -->' ) ),
			'comment_field' => '<div class="comment-form-comment">' .
						'<label for="url">' . __( 'Comment Here', 'book-store' ) . '</label>' .
						'<textarea id="comment" name="comment" aria-required="true"></textarea>' .
						'</div><!-- #form-section-comment .form-section -->',
		'comment_notes_before' => '',
		'comment_notes_after' => '',
		'title_reply' => __('Leave a Reply','book-store'),
	);
