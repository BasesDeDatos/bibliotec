<?php
	...
	*/
	...
	function print_price_item($item_xml){
		...
		foreach($price_posts as $price_post){
			...
			if( !empty($price_url) ){
				...
				echo '<a class="cp-button" href="' . $price_url . '">' . __('Read More','book-store') . '</a>';
				echo '</div>';
			}
			echo '</div>';
			echo '</div>';
	...
	function print_text_widget($item_xml){
		...
		echo '<span class="shop-btn"><a href="'.$button_link.'" class="button add_to_cart_button product_type_simple">'.$button_title.'</a></span>';
	}
	...
	function print_testimonial($item_xml){
		...
		if($display_type == 'Specific Testimonial'){
		  ...
		  <div class="slide">
			   <?php echo '<a href="'.get_permalink( $testimonial_post->ID ).'">'; ?>
				<div class="author-name-holder"> <?php if (!empty ($thumbnail[0])) { echo '<img  src="'. $thumbnail[0] .'" alt="'. $alt_text .'"/>';  } ?> </div></a>
					<strong class="title"><?php echo '<a href="'.get_permalink($testimonial_post->ID).'">' .$testimonial_post->post_title .'</a>'?><span>
	...
	function print_page_item($item_xml){
		...
		while( have_posts() ){
			...
			if( find_xml_value($item_xml, "read-more") == "Yes" ){
				echo '<a href="' . get_permalink() . '" class="portfolio-read-more cp-button">' . __('Read More','book-store') . '</a>';
			}
			echo '</div>';
