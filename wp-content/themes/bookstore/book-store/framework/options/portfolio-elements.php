	...
	function print_portfolio($item_xml){
		...
		if( $filterable == "Yes" ){
			...
			if($view_all_portfolio != 'No'){
				...
				echo '<a class="view-all" href="' . $view_all_portfolio_link . '">' . __('View All','book-store') . '</a>';
			}
		...
		if( $sidebar == "no-sidebar" ){
			...
			if( find_xml_value($item_xml, "read-more") == "Yes" ){
				echo '<a href="' . get_permalink() . '" class="cp-button">' . __('+ More','book-store') . '</a>';
			}
			...
			if( find_xml_value($item_xml, "visit-website") == "Yes" ){
				...
				if( !empty( $website ) ){
					echo '<a href="' . $website . '" class="portfolio-visit-website">' . __('Visit Website','book-store') . '</a>';
				}
			}
