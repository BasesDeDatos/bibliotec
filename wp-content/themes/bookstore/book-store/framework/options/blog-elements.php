	...
	*/
		...
		while( have_posts() ){
	        ...
	        echo '<article class="b-post">';
                ...
                echo '<ul class="post-nav">';
                 echo '<li class="blog-thumbnail-author"> ' . __('Posted by','book-store') . ' ' . get_the_author_link() . '</li>';	
                    echo '<li>on ';
					     the_date();
						 echo '</li>';
                        echo '<li>'. comments_popup_link( __('0 Comment','book-store'),
				              __('1 Comment','book-store'),
			                  __('% Comments','book-store'), '',
				              __('Comments are off','book-store') );
			            echo '</li>';
							the_tags('<li>', ', ', '</li>');
							echo '<li>';
                    ...
                    echo '</ul>';
                  echo '<a class="more-btn" href="' . get_permalink() . '">' . __('Read More','book-store') . '</a>';	
                echo '</div>';
            echo '</article>';
	...
	function print_blog_medium( $item_class, $item_size, $item_index, $num_excerpt ){
		...
		while( have_posts() ){
			...
			echo  mb_substr( get_the_excerpt(), 0, $num_excerpt ) ;	
			echo '</p>';
			echo '<a class="blog-continue-reading" href="' . get_permalink() . '"><em>' . __(' Continue Reading &rarr;','book-store') . '</em></a>';
			echo '</a>';
			echo '</div>';
			echo '</div>';
			...
			echo '<ul>';
			echo '<li class="blog-thumbnail-author"> ' . __('by','book-store') . ' ' . get_the_author_link() . '</li>';	
			/*echo '<li class="blog-thumbnail-comment">';
			comments_popup_link( __('0 Comment','book-store'),
				__('1 Comment','book-store'),
				__('% Comments','book-store'), '',
				__('Comments are off','book-store') );
			echo '</li>';*/
