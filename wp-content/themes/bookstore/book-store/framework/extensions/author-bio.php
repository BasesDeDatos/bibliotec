  ...
  Class cp_Author_box_red {
    ...
    echo "<div class='about-author-wrapper'>";
		 echo "<div class='about-author-avartar'>" . get_avatar( get_the_author_meta('ID'), 90 ) . "</div>";
						...
						echo "<div class='about-author-title cp-link-title cp-title'>" . __('About the Author','book-store') . "</div>";
...
<?php $bab_showauttxt = get_option('bab_showauttxt');
                        ...
                        <ul class="author_links">
                          ...
                          <li class="blog"><a href="<?php echo get_the_author_meta('url'); ?>" title="<?php echo esc_attr(sprintf(__('Read %s&#8217;s blog', 'book-store'), $author['name'])); ?>"><?php echo __('Blog','crunchpress'); ?></a></li>
                          <?php if(!empty($author['twitter'])): ?>
                          <li class="twitter"><a href="<?php echo $author['twitter']; ?>" title="<?php echo esc_attr(sprintf(__('Follow %s on Twitter', 'book-store'), $author['name'])); ?>" rel="external">Twitter</a></li>
                          <?php endif; ?>
                          <?php if(!empty($author['facebook'])): ?>
                          <li class="facebook"><a href="<?php echo $author['facebook']; ?>" title="<?php echo esc_attr(sprintf(__('Be %s&#8217;s friend on Facebook', 'book-store'), $author['name'])); ?>" rel="external">Facebook</a></li>
                          <?php endif; ?>
                          <?php if(!empty($author['gplus'])): ?>
                          <li class="gplus"><a href="<?php echo $author['gplus']; ?>" rel="me" title="<?php echo esc_attr(sprintf(__('Add %s in your circle', 'book-store'), $author['name'])); ?>" rel="external">Google+</a></li>
                          <?php endif; ?>
                          <?php if(!empty($author['lknin'])): ?>
                          <li class="linkin"><a href="<?php echo $author['lnkin']; ?>"  title="<?php echo esc_attr(sprintf(__('Connect with %s', 'book-store'), $author['name'])); ?>" rel="external">LinkedIn</a></li>
                          <?php endif; ?>
                        </ul>
