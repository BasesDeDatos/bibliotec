<?php
//mod by tt
//change from Class book-store to TT_SEO_MODULE
class TT_SEO_MODULE{
	...
	//mod by tt
	//change from function book-store to TT_SEO_MODULE
	...
	function init() {
		if (function_exists('load_plugin_textdomain')) {
			...
			//suppress by TT
			//	load_plugin_textdomain('book-store', str_replace( ABSPATH, '', dirname(__FILE__)));
			} else {
			//suppress by TT
			//	load_plugin_textdomain('book-store', false, dirname(plugin_basename(__FILE__)));
			}
		}
	}
	...
	function wp_head() {
		...
		if ($aioseop_options['aiosp_rewrite_titles']) {
			...
			if (sizeof($active_handlers) > 0 &&
				...
				strtolower('book-store::output_callback_for_title')) {
				ob_end_flush();
			} else {
...
/**
	    ...
	    if ($file_content === false) {
	    	$this->upgrade_error = sprintf(__("Could not download distribution (%s)","book-store"), $this->upgrade_url);
			$success = false;
	    } else if (strlen($file_content) < 100) {
	    	$this->upgrade_error = sprintf(__("Could not download distribution (%s): %s","book-store"), $this->upgrade_url, $file_content);
			$success = false;
	    } else {
		    ...
		    $this->log("fh is $fh");
		    if (!$fh) {
		    	$this->upgrade_error = sprintf(__("Could not open %s for writing","book-store"), $this->upgrade_filename);
		    	$this->upgrade_error .= "<br />";
		    	$this->upgrade_error .= sprintf(__("Please make sure %s is writable","book-store"), $this->upgrade_folder);
		    	$success = false;
		    } else {
		    	$bytes_written = @fwrite($fh, $file_content);
			    $this->log("wrote $bytes_written bytes");
		    	if (!$bytes_written) {
			    	$this->upgrade_error = sprintf(__("Could not write to %s","book-store"), $this->upgrade_filename);
			    	$success = false;
		    	}
		    }
		    if ($success) {
...
//		if (!wp_verify_nonce($nonce, 'edit-aioseop-nonce')) die ( 'Security Check - If you receive this in error, log out and back in to WordPress');
	...
	function edit_category_form() {
		...
		<?php _e('SEO Module', 'book-store') ?>
		</th>
		</tr>
		<tr>
		<th width="33%" scope="row" valign="top"><label for="aiosp_title"><?php _e('Title:', 'book-store') ?></label></th>
		<td><input value="<?php echo $title ?>" type="text" name="aiosp_title" size="70"/></td>
		</tr>
		<tr>
		<th width="33%" scope="row" valign="top"><label for="aiosp_keywords"><?php _e('Keywords (comma separated):', 'book-store') ?></label></th>
	...
	function add_meta_tags_textinput() {
	 ...
	 <?php if (substr($this->wp_version, 0, 3) >= '2.5') { ?>
                <div id="postaiosp" class="postbox closed">
                	<h3><?php _e('All in One SEO Pack', 'book-store') ?></h3>
                	<div class="inside">
                		<div id="postaiosp">
                <?php } else { ?>
                			<div class="dbx-b-ox-wrapper">
                				<fieldset id="seodiv" class="dbx-box">
                					<div class="dbx-h-andle-wrapper">
                						<h3 class="dbx-handle"><?php _e('All in One SEO Pack', 'book-store') ?></h3>
                					</div>
                					<div class="dbx-c-ontent-wrapper">
                						<div class="dbx-content">
                <?php } ?>
	
				<a target="__blank" href="http://semperfiwebdesign.com/gallery/wordpress/wordpress-plugins/all-in-one-seo-pack/"><?php _e('Click here for Support', 'book-store') ?></a>
				<input value="aiosp_edit" type="hidden" name="aiosp_edit" />
				<table style="margin-bottom:40px">
					...
					<tr>
						<th scope="row" style="text-align:right;"><?php _e('Title:', 'book-store') ?></th>
						<td><input value="<?php echo $title ?>" type="text" name="aiosp_title" size="62"/></td>
					</tr>
					<tr>
						<th scope="row" style="text-align:right;"><?php _e('Description:', 'book-store') ?></th>
						<td><textarea name="aiosp_description" rows="1" cols="60" onKeyDown="countChars(document.post.aiosp_description,document.post.length1)"	onKeyUp="countChars(document.post.aiosp_description,document.post.length1)"><?php echo $description ?>
							...
							<?php _e(' characters. Most search engines use a maximum of 160 chars for the description.', 'book-store') ?>
						</td>
					</tr>
					<tr>
						<th scope="row" style="text-align:right;"><?php _e('Keywords (comma separated):', 'book-store') ?></th>
					...
					<?php if ($this->is_admin()) { ?>
						<tr>
							<th scope="row" style="text-align:right; vertical-align:top;">
								<?php _e('Disable on this page/post:', 'book-store')?>
							</th>
							<td>
						...
						<tr>
							<th scope="row" style="text-align:right;"><?php _e('Title Attribute:', 'book-store') ?></th>
							<td><input value="<?php echo $aiosp_titleatr ?>" type="text" name="aiosp_titleatr" size="62"/></td>
						</tr>
						<tr>
							<th scope="row" style="text-align:right;"><?php _e('Menu Label:', 'book-store') ?></th>
...
<?php
	...
	function options_panel() {
		$message = null;
		//$message_updated = __("All in One SEO Options Updated.", 'book-store');
		global $aioseop_options;
		...
		if(!$aioseop_options['aiosp_cap_cats']) {
			...
			if (isset($_POST['action']) && $_POST['action'] == 'aiosp_update' && isset($_POST['Submit_Default'])) {
				...
				$message = __("SEO Settings Reset.", 'book-store');
		...
		if(isset($_POST['action'])&&isset($_POST['Submit'])){
			if ($_POST['action'] && $_POST['action'] == 'aiosp_update' && $_POST['Submit']!='') {
				$nonce = $_POST['nonce-aioseop'];
				if (!wp_verify_nonce($nonce, 'aioseop-nonce')) die ( 'Security Check - If you receive this in error, log out and back in to WordPress');
				$message = __("SEO Settings Updated.", 'book-store');
			...
			$message = __("Upgraded to newest version. Please revisit the options page to make sure you see the newest version.", 'book-store');
			$success = $this->install_newest_version();
			if (!$success) {
				$message = __("Upgrade failed", 'book-store');
...
?>
	...
	<?php
	   If (is_plugin_active('all-in-one-seo-pack/book-store.php')) {
	...
	<tr>
	<th scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Plugin Status:', 'book-store')?>
	</a>
	</td>
	<td>
	<input type="radio" name="aiosp_enabled" value="1" <?php if($aioseop_options['aiosp_enabled']) echo "checked"?> > <?php _e('Enabled', 'book-store'); ?><br>
	<input type="radio" name="aiosp_enabled" value="0" <?php if(!$aioseop_options['aiosp_enabled']) echo "checked"?>> <?php _e('Disabled', 'book-store'); ?>
	<div style="max-width:500px; text-align:left; display:none" id="aiosp_enabled_tip">
	<?php
	_e('All in One SEO Pack must be enabled for use.', 'book-store');
	 ?>
	</div>
	</td>
	</tr>
	...
	<tr>
	<th scope="row" style="text-align:right; vertical-align:top;">
	<a style="cursor:pointer;" title="<?php _e('Click for Help!', 'book-store'); ?>" onclick="toggleVisibility('aiosp_enabled_tip');">
	<?php _e('Plugin Status:', 'book-store')?>
	</a>
	</td>
	<td>
	...
	<?php
	_e('The &lt;title&gt; of your homepage. This is independent of any other option. If not set, the default blog title will get used.', 'book-store');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Home Description:', 'book-store')?>
	</td>
	<td>
	...
	<span class="description">
	<?php
	_e('The &lt;meta&gt; description for your homepage. Independent of any other options, the default is no &lt;meta&gt; description at all if this is not set.', 'book-store');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Home Keywords (comma separated):', 'book-store')?>
	</td>
	<td>
	...
	<span class="description">
	<?php
	_e("A comma separated list of your most important keywords for your site that will be written as &lt;meta&gt; keywords on your homepage.", 'book-store');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Canonical URLs:', 'book-store')?>
	</td>
	<td>
	...
	<span class="description">
	<?php
	_e("This option will automatically generate Canonical URLS for your entire WordPress installation.  This will help to prevent duplicate content penalties by <a href='http://googlewebmastercentral.blogspot.com/2009/02/specify-your-canonical.html' target='_blank'>Google</a>.", 'book-store');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Rewrite Titles:', 'book-store')?>
	</td>
	<td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Post Title Format:', 'book-store')?>
	</td>
	<td>
	...
	<span class="description">
	<?php
	_e('The following macros are supported:', 'book-store');
	echo('<ul>');
	echo('<li>'); _e('%blog_title% - Your blog title', 'book-store'); echo('</li>');
	echo('<li>'); _e('%blog_description% - Your blog description', 'book-store'); echo('</li>');
	echo('<li>'); _e('%post_title% - The original title of the post', 'book-store'); echo('</li>');
	echo('<li>'); _e('%category_title% - The (main) category of the post', 'book-store'); echo('</li>');
	echo('<li>'); _e('%category% - Alias for %category_title%', 'book-store'); echo('</li>');
	echo('<li>'); _e("%post_author_login% - This post's author' login", 'book-store'); echo('</li>');
	echo('<li>'); _e("%post_author_nicename% - This post's author' nicename", 'book-store'); echo('</li>');
	echo('<li>'); _e("%post_author_firstname% - This post's author' first name (capitalized)", 'book-store'); echo('</li>');
	echo('<li>'); _e("%post_author_lastname% - This post's author' last name (capitalized)", 'book-store'); echo('</li>');
	echo('</ul>');
	 ?>
	</span>
	<p></p>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Page Title Format:', 'book-store')?>
	</td>
	<td>
	...
	echo('<li>'); _e('%blog_title% - Your blog title', 'book-store'); echo('</li>');
	echo('<li>'); _e('%blog_description% - Your blog description', 'book-store'); echo('</li>');
	echo('<li>'); _e('%page_title% - The original title of the page', 'book-store'); echo('</li>');
	echo('<li>'); _e("%page_author_login% - This page's author' login", 'book-store'); echo('</li>');
	echo('<li>'); _e("%page_author_nicename% - This page's author' nicename", 'book-store'); echo('</li>');
	echo('<li>'); _e("%page_author_firstname% - This page's author' first name (capitalized)", 'book-store'); echo('</li>');
	echo('<li>'); _e("%page_author_lastname% - This page's author' last name (capitalized)", 'book-store'); echo('</li>');
	echo('</ul>');
	 ?>
	</span>
	<p></p>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Category Title Format:', 'book-store')?>
	</td>
	<td>
	...
	echo('<li>'); _e('%blog_title% - Your blog title', 'book-store'); echo('</li>');
	echo('<li>'); _e('%blog_description% - Your blog description', 'book-store'); echo('</li>');
	echo('<li>'); _e('%category_title% - The original title of the category', 'book-store'); echo('</li>');
	echo('<li>'); _e('%category_description% - The description of the category', 'book-store'); echo('</li>');
	echo('</ul>');
	 ?>
	</span>
	<p></p>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Archive Title Format:', 'book-store')?>
	</td>
	<td>
	...
	echo('<li>'); _e('%blog_title% - Your blog title', 'book-store'); echo('</li>');
	echo('<li>'); _e('%blog_description% - Your blog description', 'book-store'); echo('</li>');
	echo('<li>'); _e('%date% - The original archive title given by wordpress, e.g. "2007" or "2007 August"', 'book-store'); echo('</li>');
	echo('</ul>');
	 ?>
	</span>
	<p></p>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Tag Title Format:', 'book-store')?>
	</td>
	<td>
	...
	echo('<li>'); _e('%blog_title% - Your blog title', 'book-store'); echo('</li>');
	echo('<li>'); _e('%blog_description% - Your blog description', 'book-store'); echo('</li>');
	echo('<li>'); _e('%tag% - The name of the tag', 'book-store'); echo('</li>');
	echo('</ul>');
	 ?>
	</span>
	<p></p>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Search Title Format:', 'book-store')?>
	</td>
	<td>
	...
	echo('<li>'); _e('%blog_title% - Your blog title', 'book-store'); echo('</li>');
	echo('<li>'); _e('%blog_description% - Your blog description', 'book-store'); echo('</li>');
	echo('<li>'); _e('%search% - What was searched for', 'book-store'); echo('</li>');
	echo('</ul>');
	 ?>
	</span>
	<p></p>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Description Format:', 'book-store')?>
	</td>
	<td>
	...
	echo('<li>'); _e('%blog_title% - Your blog title', 'book-store'); echo('</li>');
	echo('<li>'); _e('%blog_description% - Your blog description', 'book-store'); echo('</li>');
	echo('<li>'); _e('%description% - The original description as determined by the plugin, e.g. the excerpt if one is set or an auto-generated one if that option is set', 'book-store'); echo('</li>');
	echo('<li>'); _e('%wp_title% - The original wordpress title, e.g. post_title for posts', 'book-store'); echo('</li>');
	echo('</ul>');
	 ?>
	</span>
	<p></p>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('404 Title Format:', 'book-store')?>
	</td>
	<td>
	...
	echo('<li>'); _e('%blog_title% - Your blog title', 'book-store'); echo('</li>');
	echo('<li>'); _e('%blog_description% - Your blog description', 'book-store'); echo('</li>');
	echo('<li>'); _e('%request_url% - The original URL path, like "/url-that-does-not-exist/"', 'book-store'); echo('</li>');
	echo('<li>'); _e('%request_words% - The URL path in human readable form, like "Url That Does Not Exist"', 'book-store'); echo('</li>');
	echo('<li>'); _e('%404_title% - Additional 404 title input"', 'book-store'); echo('</li>');
	echo('</ul>');
	 ?>
	</span>
	<p></p>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Paged Format:', 'book-store')?>
	</td>
	<td>
	...
	<span  class="description">
	<?php
	_e('This string gets appended/prepended to titles when they are for paged index pages (like home or archive pages).', 'book-store');
	_e('The following macros are supported:', 'book-store');
	echo('<ul>');
	echo('<li>'); _e('%page% - The page number', 'book-store'); echo('</li>');
	echo('</ul>');
	 ?>
	</span>
	<p></p>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('SEO for Custom Post Types:', 'book-store')?>
	</td>
	<td>
	...
	<span class="description">
	<?php
	_e('Check this if you want your enable AIOSEOP support for Custom Post Types on this site.', 'book-store');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Custom Post Types for SEO Column Support:', 'book-store')?>
	</td>
	<td>
	...
	<span class="description">
	<?php
	_e('Choose which post types you want to have SEO columns on the edit.php screen. You can select as many as you like.', 'book-store');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Use Categories for META keywords:', 'book-store')?>
	</td>
	<td>
	...
	<span class="description">
	<?php
	_e('Check this if you want your categories for a given post used as the META keywords for this post (in addition to any keywords and tags you specify on the post edit page).', 'book-store');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Use Tags for META keywords:', 'book-store')?>
	</td>
	<td>
	...
	<span class="description">
	<?php
	_e('Check this if you want your tags for a given post used as the META keywords for this post (in addition to any keywords you specify on the post edit page).', 'book-store');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Dynamically Generate Keywords for Posts Page:', 'book-store')?>
	</td>
	<td>
	...
	<span class='description'>
	<?php
	_e('Check this if you want your keywords on a custom posts page (set it in options->reading) to be dynamically generated from the keywords of the posts showing on that page.  If unchecked, it will use the keywords set in the edit page screen for the posts page.', 'book-store');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Use noindex for Categories:', 'book-store')?>
	</td>
	<td>
	...
	<span class="description">
	<?php
	_e('Check this for excluding category pages from being crawled. Useful for avoiding duplicate content.', 'book-store');
	 ?>
	</div>
	</span>
	<p></p>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Use noindex for Archives:', 'book-store')?>
	</td>
	<td>
	...
	<span class="description">
	<?php
	_e('Check this for excluding archive pages from being crawled. Useful for avoiding duplicate content.', 'book-store');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Use noindex for Tag Archives:', 'book-store')?>
	</td>
	<td>
	...
	<span class="description">
	<?php
	_e('Check this for excluding tag pages from being crawled. Useful for avoiding duplicate content.', 'book-store');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Autogenerate Descriptions:', 'book-store')?>
	</td>
	<td>
	<input type="checkbox" name="aiosp_generate_descriptions" <?php if ($aioseop_options['aiosp_generate_descriptions']) echo "checked=\"1\""; ?>/>
	<span class="">
	<?php
	_e("Check this and your META descriptions will get autogenerated if there's no excerpt.", 'book-store');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Capitalize Category Titles:', 'book-store')?>
	</td>
	<td>
	...
	<span class="description">
	<?php
	_e("Check this and Category Titles will have the first letter of each word capitalized.", 'book-store');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Exclude Pages:', 'book-store')?>
	</td>
	<td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Additional Post Headers:', 'book-store')?>
	</td>
	<td>
	...
	<span class="description">
	<?php
	_e('What you enter here will be copied verbatim to your header on post pages. You can enter whatever additional headers you want here, even references to stylesheets.', 'book-store');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Additional Page Headers:', 'book-store')?>
	</td>
	<td>
	...
	<span class="description">
	<?php
	_e('What you enter here will be copied verbatim to your header on pages. You can enter whatever additional headers you want here, even references to stylesheets.', 'book-store');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<tr>
	<td scope="row" style="text-align:right; vertical-align:top;">
	<?php _e('Additional Home Headers:', 'book-store')?>
	</td>
	<td>
	...
	<span class="description">
	<?php
	_e('What you enter here will be copied verbatim to your header on the home page. You can enter whatever additional headers you want here, even references to stylesheets.', 'book-store');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<tr>
	<th scope="row" style="text-align:right; vertical-align:top;">
	<a style="cursor:pointer;" title="<?php _e('Click for Help!', 'book-store')?>" onclick="toggleVisibility('aiosp_do_log_tip');">
	<?php _e('Log important events:', 'book-store')?>
	</a>
	</td>
	<td>
	...
	<?php
	_e('Check this and SEO pack will create a log of important events (book-store.log) in its plugin directory which might help debugging it. Make sure this directory is writable.', 'book-store');
	 ?>
	</div>
	</td>
	</tr>
	...
	<input type="submit" class='button-primary' name="Submit" value="<?php _e('Update Settings', 'book-store')?>" /> 
	<input type="submit" class='button-primary' name="Submit_Default" value="<?php _e('Reset Settings to Defaults', 'book-store')?>" /> 
	</p>
	<?php } ?>
	</form>
	</div>
