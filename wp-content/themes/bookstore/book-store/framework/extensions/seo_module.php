...
/*
* Modifications done as book-store extension.
* @since version 2.6
* List of mods includes the following
*
* 1) suppress all admin notices.
* 2) Changed WP_PLUGIN_DIR to use book-store_EXTENDED
...
* 10) Renamed file - book-store.php to seo_module.php
...
*/
	...
	//mod by tt to use book-store_EXTENDED instead of  WP_PLUGIN_DIR
	...
	if( array_key_exists('aiosp_enabled', $aioseop_options) ){
	...
	*/
	...
	if($aioseopcc){
		if(tt_aioseop_get_version() != trim(wp_remote_fopen('http://aioseoppro.semperfiwebdesign.com/version.html'))){
			add_action('after_plugin_row_all-in-one-seo-pack-pro/book-store.php', 'tt_add_plugin_row', 10, 2);
		}
	}
	**/
	...
	function tt_aioseop_mrt_pcolumns($aioseopc) {
		$aioseopc['seotitle'] = __('SEO Title','book-store');
		$aioseopc['seokeywords'] = __('SEO Keywords','book-store');
		$aioseopc['seodesc'] = __('SEO Description','book-store');
		return $aioseopc;
	}
	...
	function tt_aiosp_meta() {
		...
		<!--suppress by tt	
			<a target="__blank" href="http://semperfiwebdesign.com/forum/"><?php _e('Click here for Support', 'book-store') ?></a>-->
		  ...
		  <table style="margin-bottom:40px">
			...
			<th scope="row" style="text-align:right;"><?php _e('Title:', 'book-store') ?></th>
				<td>
					...
					<?php _e(' characters. Most search engines use a maximum of 60 chars for the title.', 'book-store') ?>
				</td>
			</tr>
