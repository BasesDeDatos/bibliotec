...
function gdl_image_resize( $file, $max_w, $max_h, $crop = false, $suffix = null, $dest_path = null, $jpeg_quality = 100 ) {
	...
	if ( !$size )
		return new WP_Error('invalid_image', __('Could not read image size','book-store'), $file);
	list($orig_w, $orig_h, $orig_type) = $size;

	$dims = gdl_image_resize_dimensions($orig_w, $orig_h, $max_w, $max_h, $crop);
	if ( !$dims )
		return new WP_Error( 'error_getting_dimensions', __('Could not calculate resized image dimensions','book-store') );
	...
	if ( IMAGETYPE_GIF == $orig_type ) {
		if ( !imagegif( $newimage, $destfilename ) )
			return new WP_Error('resize_path_invalid', __( 'Resize path invalid','book-store'));
